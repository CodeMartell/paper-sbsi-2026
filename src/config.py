...
class Settings:
    ...
    gerp_url: str = field(default_factory=lambda: os.getenv("GERP_URL", "http://localhost:8000/gerp_fake.html"))
    gerp_user: str = field(default_factory=lambda: os.getenv("GERP_USER", "aluno"))
    gerp_password: str = field(default_factory=lambda: os.getenv("GERP_PASSWORD", "avaliacao2026"))

    # Fuso horário
    tz: str = field(default_factory=lambda: os.getenv("TZ", "America/Manaus"))

    # Diretórios
    data_dir: Path = field(default_factory=lambda: Path(os.getenv("DATA_DIR", str(_base_dir() / "data"))))
    output_dir: Path = field(default_factory=lambda: Path(os.getenv("OUTPUT_DIR", str(_base_dir() / "output"))))
    logs_dir: Path = field(default_factory=lambda: Path(os.getenv("LOGS_DIR", str(_base_dir() / "logs"))))

    # Nomes de arquivo
    financeiro_csv: str = field(default_factory=lambda: os.getenv("FINANCEIRO_CSV", "GERP_faturamento_bruto.csv"))
    producao_xlsx: str = field(default_factory=lambda: os.getenv("PRODUCAO_XLSX", "producao_fisica_real.xlsx"))
    template_relatorio: str = field(
        default_factory=lambda: os.getenv("TEMPLATE_RELATORIO", "modelo_relatorio_final_diretoria.txt")
    )

    # Regras de negócio (limiares em %)
    limiar_desvio_financeiro_atencao: float = field(
        default_factory=lambda: float(os.getenv("LIMIAR_DESVIO_FINANCEIRO_ATENCAO", "5"))
    )
